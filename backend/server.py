...
def generate_contract_pdf(contract_data: dict, signature_base64: str = None) -> bytes:
    """Generate a PDF contract from contract data"""
    ...
    styles.add(ParagraphStyle(name='FieldValue', fontSize=10, fontName='Helvetica'))
    ...
    for cond in tpl["conditions"]:
        story.append(Paragraph(f"• {cond}", styles['ContractBody']))
    story.append(Spacer(1, 5*mm))
    if signature_base64:
        try:
...
@api_router.post("/admin/contracts/generate")
async def generate_contract(data: ContractGenerate, user: dict = Depends(get_admin_user)):
    """Admin generates a contract for a reservation"""
...
async def get_contract(contract_id: str, user: dict = Depends(get_current_user)):
    ...
    # Access check: admin of same agency, or the client themselves
...
@api_router.get("/contracts/by-reservation/{reservation_id}")
async def get_contract_by_reservation(reservation_id: str, user: dict = Depends(get_current_user)):
    """Get contract for a specific reservation"""
...
@api_router.put("/contracts/{contract_id}/send")
async def send_contract(contract_id: str, user: dict = Depends(get_admin_user)):
    """Admin marks contract as sent to client"""
    contract = await db.contracts.find_one({"id": contract_id}, {"_id": 0})
    if not contract:
        raise HTTPException(status_code=404, detail="Contract not found")
    ...
    if client:
        msg_fr = "Un contrat de location est prêt pour votre signature."
        msg_en = "A rental contract is ready for your signature."
        msg = msg_fr if contract.get("language", "fr") == "fr" else msg_en
        await create_notification(contract["user_id"], "contract", msg, contract_id)
    return {"message": "Contrat envoyé au client"}


@api_router.put("/contracts/{contract_id}/sign")
...
async def download_contract_pdf(contract_id: str, user: dict = Depends(get_current_user)):
    """Generate and return PDF of contract"""
