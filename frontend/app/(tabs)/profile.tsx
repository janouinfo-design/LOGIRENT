...
export default function ProfileScreen() {
  ...
  const downloadContractPdf = async (contractId: string) => {
    try {
      ...
      Platform.OS === 'web' ? window.alert('Erreur telechargement PDF') : Alert.alert(t("Erreur"));
    }
  };
  ...
  const viewContract = async (contractId: string) => {
    try {
      ...
      Platform.OS === 'web' ? window.alert('Erreur visualisation contrat') : Alert.alert(t("Erreur"));
    }
  };
  ...
  if (isLoading || !user) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>{t("Chargement du profil...")}</Text>
      </View>
    );
  }
  ...
  const handleLogout = async () => {
    if (Platform.OS === 'web') {
      ...
      Alert.alert(t("Déconnexion"), t("Êtes-vous sûr de vouloir vous déconnecter ?"), [
        ...
        { text: t("Déconnexion"), style: 'destructive', onPress: async () => { await logout(); } },
      ]);
    }
  };
  ...
  const showVerificationResult = (v: any) => {
    ...
    const statusIcon = v.is_valid ? t("Document valide") : t("Document rejeté");
  ...
  const takePhoto = async (type: 'id' | 'license' | 'id_back' | 'license_back') => {
    ...
    if (status !== 'granted') { Alert.alert(t("Permission requise"), t("Veuillez autoriser l'accès à la caméra.")); return; }
    try {
      ...
      Alert.alert(t("Erreur"), t("Impossible d'ouvrir la caméra."));
    }
  };

  const pickFromGallery = async (type: 'id' | 'license' | 'id_back' | 'license_back') => {
    const { status } = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (status !== 'granted') { Alert.alert(t("Permission requise"), t("Veuillez autoriser l'accès à la galerie.")); return; }
    try {
      ...
      Alert.alert(t("Erreur"), t("Impossible d'ouvrir la galerie."));
    }
  };
  ...
  const menuItems = [
    { icon: 'receipt', label: t("Mes factures"), action: () => router.push('/my-invoices' as any) },
    { icon: 'scan', label: t("Mes documents"), action: () => router.push('/my-documents' as any) },
    { icon: 'card', label: t("Moyens de paiement") },
    { icon: 'notifications', label: 'Notifications' },
    { icon: 'shield-checkmark', label: t("Confidentialité") },
    { icon: 'help-circle', label: t("Aide & Support") },
    { icon: 'document-text', label: t("Conditions d'utilisation") },
  ];
      ...
      <View style={styles.content}>
        ...
        <View style={[styles.header, { backgroundColor: C.bg }]}>
          ...
          {!editing && (
            <TouchableOpacity style={styles.editBtn} onPress={() => setEditing(true)} data-testid="edit-profile-btn">
              ...
              <Text style={styles.editBtnText}>{t("Modifier le profil")}</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        {editing && (
          <View style={styles.section}>
            <Input label={t("Nom complet")} value={name} onChangeText={setName} icon="person" />
            <Input label={t("Téléphone")} value={phone} onChangeText={setPhone} keyboardType="phone-pad" icon="call" />
            <Input label={t("Adresse")} value={address} onChangeText={setAddress} icon="location" />

            <View style={styles.identitySection}>
              <View style={styles.identitySectionHeader}>
                <Ionicons name="id-card" size={16} color={_C.purple} />
                <Text style={styles.identitySectionTitle}>{t("Identité & Permis")}</Text>
              </View>

              <View style={styles.fieldRow}>
                <View style={{ flex: 1 }}>
                  <Input label={t("Lieu de naissance")} value={birthPlace} onChangeText={setBirthPlace} icon="location-outline" />
                </View>
                <View style={{ flex: 1 }}>
                  <Input label={t("Date de naissance (JJ-MM-AAAA)")} value={dateOfBirth} onChangeText={(v: string) => setDateOfBirth(formatDateInput(v))} icon="calendar-outline" />
                </View>
              </View>

              <Input label={t("Nationalité")} value={nationality} onChangeText={setNationality} icon="globe-outline" />
              <Input label={t("N° Permis de conduire")} value={licenseNumber} onChangeText={setLicenseNumber} icon="car-outline" />

              <View style={styles.fieldRow}>
                <View style={{ flex: 1 }}>
                  <Input label={t("Date d'émission permis")} value={licenseIssueDate} onChangeText={(v: string) => setLicenseIssueDate(formatDateInput(v))} icon="calendar-outline" />
                </View>
                <View style={{ flex: 1 }}>
                  <Input label={t("Date d'expiration permis")} value={licenseExpiryDate} onChangeText={(v: string) => setLicenseExpiryDate(formatDateInput(v))} icon="calendar-outline" />
                </View>
              </View>
            </View>

            <View style={styles.btnRow}>
              <Button title={t("Annuler")} onPress={() => setEditing(false)} variant="outline" style={{ flex: 1 }} />
              <Button title={t("Enregistrer")} onPress={handleSave} loading={loading} style={{ flex: 1 }} />
            </View>
          </View>
        )}
        ...
        {/* Documents */}
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: C.text }]}>{t("Mes Reservations")}</Text>
          {loadingReservations ? (
            ...
            <View style={[styles.infoCard, { backgroundColor: C.card, borderColor: C.border, alignItems: 'center', padding: 20 }]}>
              ...
              <Text style={{ color: C.textLight, fontSize: 13, marginTop: 8 }}>{t("Aucune reservation")}</Text>
            </View>
          ) : (
        ...
        {/* Documents */}
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: C.text }]}>{t("Documents Obligatoires")}</Text>
          ...
          {(!user.id_photo || !user.license_photo) && (
            <View style={styles.alert}>
              ...
              <Text style={styles.alertText}>{t("Documents requis pour effectuer une réservation. Formats acceptés: JPG, PNG, WebP (max 10 MB).")}</Text>
            </View>
          )}
          ...
          <View style={[styles.docFullCard, { backgroundColor: C.card, borderColor: C.border }]}>
            <View style={styles.docHeader}>
              ...
              <Text style={[styles.docTitle, { color: C.text }]}>{t("Pièce d'Identité")}</Text>
          ...
          <View style={[styles.docFullCard, { backgroundColor: C.card, borderColor: C.border, marginTop: 12 }]}>
            <View style={styles.docHeader}>
              ...
              <Text style={[styles.docTitle, { color: C.text }]}>{t("Permis de Conduire")}</Text>
        ...
        <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginHorizontal: 20, marginBottom: 12, padding: 14, borderRadius: 14, gap: 8, borderWidth: 1, borderColor: '#7C3AED40', backgroundColor: '#F5F3FF' }} onPress={() => router.push('/forgot-password' as any)} data-testid="reset-password-btn">
          ...
          <Text style={{ fontSize: 14, fontWeight: '600', color: '#7C3AED' }}>{t("Reinitialiser le mot de passe")}</Text>
        </TouchableOpacity>
        ...
        <View style={uploadModalStyles.overlay}>
          <View style={uploadModalStyles.card}>
            <Text style={uploadModalStyles.title}>
              {uploadModal === 'id' ? 'Pièce d\'identité (Recto)' : uploadModal === 'id_back' ? 'Pièce d\'identité (Verso)' : uploadModal === 'license' ? t("Permis de conduire (Recto)") : t("Permis de conduire (Verso)")}
            </Text>
            <Text style={uploadModalStyles.subtitle}>{t("Comment souhaitez-vous ajouter votre document ?")}</Text>
            <TouchableOpacity style={uploadModalStyles.btn} onPress={() => handleUploadChoice('camera')}>
              <Ionicons name="camera" size={24} color="#7C3AED" />
              <Text style={uploadModalStyles.btnText}>{t("Prendre une photo")}</Text>
            </TouchableOpacity>
